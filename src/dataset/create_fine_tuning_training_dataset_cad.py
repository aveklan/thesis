...
training_file = Path(
    root_dir / "fine_tuning_datasets" / "cad_dataset_withContext_training.json"
)
testing_file = Path(
    root_dir / "fine_tuning_datasets" / "cad_dataset_withContext_testing.json"
)
# Load the JSON file
...
disability_comments = [
    item
    for item in data
    if item.get("annotation_category", "") == "ableness/disability"
]
other_comments = [
    item
    for item in data
