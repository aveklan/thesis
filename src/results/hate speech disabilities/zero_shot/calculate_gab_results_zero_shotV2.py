...
ethos_file_path = root_dir / "zero_shot" / "gab_dataset_classified_zero_shot.json"
    ...
    correct_count = sum(
        ...
        if (
            (
                ...
                and entry.get("MPH", 0) == 1.0
            )
            or (
                entry.get(model_key, "").strip().lower() in NO_VALUES
                and entry.get("MPH", 0.0) == 0.0
            )
        )
    )
    total_comments = len(comments)
    ...
    for entry in comments:
        ...
        if result in YES_VALUES:
            if disability == 1:
                ...
                false_positive += 1
        elif result in NO_VALUES and disability == 1:
            false_negative += 1
...
def ethos_results():
    ...
    disability_comments = [entry for entry in comments if entry.get("MPH", 0) == 1]
